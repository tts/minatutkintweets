...
# To do: use the Finto AI REST API to get suggested keywords
# https://www.kiwi.fi/display/Finto/Finto+AI%3An+rajapintapalvelu
# Use raw tweets but remove irrelevant ones, links, emojis, and non-chars.
...
data_fi <- data_fi %>% 
  ...
  select(user_id, created_at, text, favorite_count, retweet_count, status_url, followers_count, friends_count)

write_csv(data_fi, "cleaned_fi_tweets.csv")
